"""Render dist/index.html (August inlined) and dist/data/mMM.gz for the other months.
...
def build():
    C.DIST_DATA.mkdir(parents=True, exist_ok=True)
    ...
    for m in range(1, 13):
        ...
        (C.DIST_DATA / f"m{m:02d}.gz").write_bytes(gz)
    ...
    payload = {
        "meta": {k: meta[k] for k in ("threshold", "baseline", "trailing", "q_scale", "q_offset", "years", "fallback_cells")},
        "stats": stats,
        "geo": geo_lines(),
