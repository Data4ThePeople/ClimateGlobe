...
The tool is free to use and free to embed. It is also available as a full page at [data4thepeople.github.io/ClimateGlobe](https://data4thepeople.github.io/ClimateGlobe/dist/index.html), where deep links work: add `#month=3&year=1998` to the address to open March 1998, `hot=1` to highlight only the areas above 1.5°C, and `avg=1` for the 10-year average.
...
Those are single-month readings, and single months swing. The 10-year average of Augusts from 2017 through 2026 comes in at 1.22°C, with 60% of land and 54% of people above 1.5°C. For comparison, August 1998, a strong El Niño year that was a record at the time, had 38% of land and 23% of people above the line.
...
Three public datasets go into the globe. We do not alter any of the underlying figures. Our work is the rebasing, the summing, and the drawing.

**Temperature: NASA GISTEMP v4.** The NASA Goddard Institute for Space Studies publishes the GISS Surface Temperature Analysis, a monthly grid of temperature anomalies covering the world since January 1880. We use the 2° by 2° latitude-longitude product with 1,200 km smoothing, which combines land station records (GHCN v4) with sea surface temperatures (ERSST v5). Each cell holds one value per month: how far that month's temperature departed from the cell's own 1951-1980 average for the same calendar month. The file is free, needs no login, and is updated around the middle of each month with the prior month's data.

**Population: GHS-POP 2025.** The European Commission's Joint Research Centre publishes the Global Human Settlement Layer, a population grid built from census data and satellite imagery. We use the 2025 estimate at 30 arc-second resolution, about one kilometer at the equator. It sums to 8.19 billion people.

**Coastlines, borders, and land: Natural Earth.** The 1:110 million coastline, country boundary, and land polygon files, in the public domain.

## How we built it

### Step 1: Start with what NASA already gives you

NASA's file is a stack of 1,760 monthly maps, January 1880 through August 2026, each one 90 rows of latitude by 180 columns of longitude. Every cell is an anomaly, not a temperature: the difference between that month and the cell's 1951-1980 average for that calendar month. NASA chose 1951-1980 as its reference decades ago and has kept it for continuity. Nothing about our result depends on that choice, as the next step shows.

### Step 2: Move the baseline back to 1880-1900

The 1.5°C target is measured from the pre-industrial period, which the Intergovernmental Panel on Climate Change defines as 1850-1900. NASA's record starts in 1880, so we use 1880-1900. For the globe as a whole, the two periods differ by a few hundredths of a degree.

For every cell and every calendar month, we average the 21 values from 1880 through 1900. That average is the cell's baseline for that month. The number on the globe is NASA's anomaly minus that baseline. Because both are measured against the same 1951-1980 reference, the reference cancels out, and what remains is the difference between the month you picked and the average of that same month in 1880-1900. August is always compared with August, January with January.

The shift is not uniform. For the globe as a whole, 1880-1900 Augusts were about 0.18°C cooler than 1951-1980 Augusts. Over Europe the gap is 0.36°C, and over parts of the Arctic it is larger still. A single global offset, which is how some published series convert to pre-industrial, would miss those differences. A cell-by-cell baseline keeps them.

### Step 3: Borrow a baseline where 1880-1900 has no data

In 1880 there were no weather stations in Antarctica, few in the interior of Africa, South America, or Asia, and none in the Arctic Ocean. NASA's grid leaves those cells empty for the early years. A cell needs at least ten of the 21 baseline years to get its own baseline. That rules out 27% of cells: Antarctica, the Amazon basin, central Africa, the Sahara and Arabia, the Tibetan plateau, the Arctic coasts, and much of the Southern Ocean.

For those cells we borrow. The baseline becomes the average baseline of the cells in the same latitude band that do have data, and if an entire band has none, the nearest band that does. The effect is a flat shift applied to NASA's values in that region. The pattern you see there is NASA's. The level is approximate. The footnote on the globe says which regions this affects, and the "Honest notes" section below says what it does to the headline numbers.

### Step 4: Share of land above 1.5°C

Each of the 16,200 cells gets a land fraction, computed by rasterizing the Natural Earth land polygons at a tenth of a degree and averaging 400 of those fine cells into each 2° cell. Cells near the poles are physically much smaller than cells at the equator, so each cell is also weighted by its true area on the sphere.

The land share is then the land-and-area-weighted sum of cells above 1.5°C, divided by the same sum for every cell that has a value that month. Cells with no value are left out of both the top and the bottom of that fraction, which is why the coverage note matters in early years. Antarctica counts as land. It is 8% of the world's land area.
...
For any month and year, the 10-year view averages the ten most recent instances of that month, so August 2026 becomes the average of Augusts 2017 through 2026. A cell needs at least seven of the ten to get a value; the first years of the record show nothing until enough have accumulated. The land and population shares are recomputed from the averaged map, not averaged from the single-month shares, which is the right order of operations for a threshold.

### Step 7: The color scale

The scale is diverging and centered on zero. From zero to 1.5°C it runs from near-white through pale red to a mid red. Past 1.5°C it continues to darken, reaching the deepest red at 6°C, where it stops. Blues mirror the reds on the cool side. Values beyond plus or minus 6°C, which happen in the Arctic and Antarctic in single months, take the end color. Ten-year averages rarely reach the ends of the scale; single months in polar regions often do.
...
**Borrowed baselines cover a quarter of the land.** For Antarctica, the tropical interiors, and the Arctic coasts, the baseline is borrowed from the same latitude band (Step 3). Recomputing the August 2026 shares using only cells with their own baseline moves the land figure from 77% to 74% and the population figure from 73% to 72%. For the 10-year average the land figure moves from 60% to 63%. The headline numbers are not sensitive to it, but the map in those regions is a pattern with an uncertain level.

**The grid is coarse.** Two degrees is about 220 km at the equator, and NASA's 1,200 km smoothing spreads each station's influence further. The globe cannot show a city, a mountain range, or a coastline's microclimate. Within about 800 km of the South Pole, NASA's grid carries a single value from the one station there, which is why the pole shows as a flat disk.
...
**We did not invent the data.** Every anomaly is NASA's and every population count is the Joint Research Centre's. Our contribution is the rebasing to 1880-1900, the land and population sums, the 10-year average, the color scale, and the interactivity.

## Reproduce it yourself

The code, the build steps, and the published files are at [github.com/Data4ThePeople/ClimateGlobe](https://github.com/Data4ThePeople/ClimateGlobe). You need the NASA GISTEMP gridded file, the GHS-POP 2025 raster, the three Natural Earth files, and Python. The transformations are the eight steps above; the statistics are averages and weighted sums. If you do it and get something different from us, we want to know. Tell us, and we will look.
